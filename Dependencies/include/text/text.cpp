...
#include "text.h"
#include <stb/stb_image.h> // Sean Barrett's image loader
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
	...
	float *br_y)
{
	...
	*br_y = 0.0f;
	...
	for (int i = 0; i < len; i++)
	{
		...
		x_pos = curr_x;
		y_pos = 0.0f - ((2.0f * scale_px) / font_viewport_height) * glyph_y_offsets[ascii_code] - line_offset;
	...
	float a)
{
	...
	renderable_texts[num_render_strings].tex_id = font_texture;
...
Renderable_Text *get_text(int id)
{
	if (id < 0 || id >= num_render_strings)
	{
		fprintf(stderr, "ERROR: text id %i out of range\n", id);
		return NULL;
	}
	return &renderable_texts[id];
}
...
void draw_text(int id)
{
	...
	glBindVertexArray(renderable_texts[id].vao);

	glUniform2f(font_sp_pos_loc,
				renderable_texts[id].tl_x, renderable_texts[id].tl_y);
	glUniform4f(font_sp_text_colour_loc,
	...
	glDrawArrays(GL_TRIANGLES, 0, renderable_texts[id].point_count);
